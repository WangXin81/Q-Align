...
import string
import nltk
nltk.data.path.append('/root/autodl-tmp/vvqa_project/nltk_data')
...
def evaluate_vqa(model, dataloader, device):
    total_score_exact_global_local = 0.0
    total_score_acc07_global_local = 0.0
    total_score_acc08_global_local = 0.0
    total_score_acc09_global_local = 0.0
    ...
    with torch.no_grad():
        ...
        for batch in dataloader:
            ...
            # 模型回答
            #pred_answers_batch_global = model.process_batch(question_ids, questions)
            pred_answers_batch_global_local = model.process_batch(question_ids,questions)
